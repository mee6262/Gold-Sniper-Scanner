...
def atr(df, period=ATR_PERIOD):
    if df is None or len(df) < period + 1: return float("nan")
...
def pivots(df):
    highs,lows=[],[]
    if df is None or len(df) < MIN_STRUCTURE_BARS: return highs,lows
...
def structure(df):
    if df is None or len(df) < MIN_STRUCTURE_BARS: return {"bias":"NEUTRAL","mss":"NONE","bos":"NONE"}
...
def sweep(df):
    if df is None or len(df) < MIN_STRUCTURE_BARS: return None
